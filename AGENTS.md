...
- ❌ `ChatOpenAI(model_name=...)` → ✅ `LLM(model="ollama/qwen3:8b")`
- ❌ `Agent(llm=ChatOpenAI(...))` → ✅ `Agent(llm="ollama/qwen3:8b")` or `Agent(llm=LLM(model="..."))`
- ❌ Passing raw provider client objects → ✅ Use `crewai.LLM` wrapper

### How to verify you're using current patterns:
1. You ran the version check and docs lookup steps above before writing code
2. All LLM references use `crewai.LLM` or string shorthand (`"ollama/qwen3:8b"`)
...
# Testing & training
crewai test                           # Test crew (default: 2 iterations, ollama/qwen3:8b)
crewai test -n 5 -m ollama/qwen3:8b   # Custom iterations and model
...
├── .env                       # Runtime settings (OLLAMA_HOST, SERPER_API_KEY, etc.)
└── pyproject.toml
```
...
Variables like `{topic}` are interpolated from `crew.kickoff(inputs={"topic": "Kelp in the UK"})`.
...
agent = Agent(llm="ollama/qwen3:8b", ...)
...
Environment variable default: set `OLLAMA_HOST=http://localhost:11434` and `MODEL=ollama/qwen3:8b` in `.env`.
...
SERPER_API_KEY=...
ANTHROPIC_API_KEY=...
# Optional if you want to point CrewAI at a different local model:
...
- **Using `ChatOpenAI()`** — Always use `crewai.LLM` or string shorthand like `"ollama/qwen3:8b"`
